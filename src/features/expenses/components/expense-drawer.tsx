...
import { categoryIconSchema, saveExpenseSchema } from '@/features/budget/budget.schemas'
...
type ExpenseDrawerProps = {
  ...
  /** When provided, the drawer edits this expense instead of creating one. */
  expense?: Expense
}
  ...
  return (
    <Drawer open={open} onOpenChange={onOpenChange}>
      <DrawerContent className="max-h-[92dvh]">
        <DrawerHeader>
          <DrawerTitle>{expense ? 'Modifier la dépense' : 'Nouvelle dépense'}</DrawerTitle>
          <DrawerDescription>Elle sera déduite de ce qu'il reste ce mois-ci.</DrawerDescription>
        </DrawerHeader>
...
function ExpenseForm({ budgetId, expense, onDone }: ExpenseFormProps) {
  ...
  const form = useAppForm({
    ...
    onSubmit: ({ value }) => {
      ...
      const label = value.label.trim() || categoryName || 'Dépense'
      return save.mutateAsync({ data: { ...value, label, budgetId, id: expense?.id } }).catch(() => undefined)
    },
  })
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-2">
        ...
        <form.AppForm>
          <form.SubmitButton className="flex-1">{expense ? 'Enregistrer' : 'Ajouter la dépense'}</form.SubmitButton>
        </form.AppForm>
      </div>
    </form>
  )
